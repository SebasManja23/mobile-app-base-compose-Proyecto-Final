...
fun ReportesScreen(navController: NavHostController, viewModel: MaizViewModel) {
    ...
    val inventarioCalculado = productosNombres.map { nombreProd ->
        ...
        if (totalKg >= 1000 || totalKg <= -1000) {
            ...
            listOf(nombreProd, String.format(Locale.getDefault(), "%.1f", totalKg), "kg")
        }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            ReportSection(
                ...
                headers = listOf("Prod.", "Tipo", "Cant.", "Unid.", "Fecha"),
            ...
            ReportSection(
                ...
                headers = listOf("Producto", "Stock Total", "Unidad"),
...
fun ReportSection(title: String, icon: androidx.compose.ui.graphics.vector.ImageVector, headers: List<String>, data: List<List<String>>) {
    Column {
        Row(verticalAlignment = Alignment.CenterVertically) {
            Icon(icon, contentDescription = null, tint = Color(0xFFDAA520), modifier = Modifier.size(24.dp))
        ...
        Card(
            ...
            Column(modifier = Modifier.fillMaxWidth()) {
                Row(modifier = Modifier.fillMaxWidth().background(MaizeGreen).padding(8.dp)) {
                    headers.forEach { header ->
                        Text(text = header, modifier = Modifier.weight(1f), color = Color.White, fontSize = 10.sp, fontWeight = FontWeight.Bold)
                    }
                }
                if (data.isEmpty()) {
                    Text(text = stringResource(R.string.no_records), modifier = Modifier.padding(16.dp).align(Alignment.CenterHorizontally), color = Color.Gray, fontSize = 12.sp)
                } else {
                    data.forEachIndexed { index, row ->
                        Row(modifier = Modifier.fillMaxWidth().background(if (index % 2 == 0) Color.White else Color(0xFFF9F9F9)).padding(8.dp)) {
                            row.forEach { cell ->
                                Text(
                                    text = cell,
